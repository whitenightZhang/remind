...
run_number = "validated_bestScenario"
...
scenarios = c(
  ...
  # "REMIND_generic_oTS_INT",
  # "REMIND_generic_oTS_INT_gridmod",
  NULL)
...
vars = c(     "Emi|CO2|Industrial Processes",
...
# df.emi.grossneg <- df %>%
  # filter(variable %in% c("Emi|CO2|Energy|Demand|Industry"), period %in% plot.period, region == "CHA") %>% 
  # order.levels(variable = vars)
...
df.tot.plot =  filter(df, period <= 2080, scenario %in% scens_short) %>%
  # filter(variable == "Emi|CO2") %>% 
...
mip::mipArea(df.plot %>% filter(region == reg, scenario == scen), total = df.tot.plot %>% filter(region == reg, scenario == scen)) +
  ...
  geom_line(data= df.hist.emi, aes(x=period, y = value), color = "black", size = 1.5 )+
...
if (reg == "CHA"){
  # df.tsinhua_co2a <- data.frame(period=seq(from = 2020, to = 2070, by = 5), 
  #                  value=c(10.2,10.1,9.4,8.2,6.5,4.3,3.1,1.9,1.2,0.6,0), scenario = "Tsinghua 2C") 
  # 
  # df.tsinhua_co2b <- data.frame(period=seq(from = 2020, to = 2070, by = 5), 
  #                              value=c(10.03,9.2,7.44,6,4.2,2.9,1.47,.7,0,0,0), scenario = "Tsinghua 1.5C") 
  
  # data from Weng (PIK cloud), total CO2 including industrial process
  df.tsinhua_CGEM <- data.frame(period=seq(from = 2020, to = 2070, by = 5), 
                                value=c(11.2,11.5,11.0,9.5,7.2,4.9,3.1,1.6,0.2,0.,0), scenario = "Tsinghua netzero") 
...
variable = c("CHA	Emi|GHG|CO2 (energy activity + industrial process)", 
             "CHA	Emi|CO2|Electricity",
)
tech = c(
  "elh2",
  # Coal
...
df.linecomparison_ES_ind_totsteel <- df.linecomparison_ES_ind %>% 
  # filter(region=="CHA") %>% 
  filter(variable %in% c("Production|Industry|Steel|Primary (Mt/yr)", "Production|Industry|Steel|Secondary (Mt/yr)")) %>% 
  spread(variable, value) %>% 
  mutate(totsteel = `Production|Industry|Steel|Primary (Mt/yr)` + 
           `Production|Industry|Steel|Secondary (Mt/yr)`) %>% 
  select(scenario, model, region, period, totsteel) %>% 
  mutate(variable = "Production|Industry|Steel (Mt/yr)")

p<-ggplot() + geom_line(data= df.linecomparison_ES_ind_totsteel %>% filter(region==reg), aes(x=period, y = totsteel, color = scenario)) +
  theme_bw() +
  labs(color = "") +
  theme(strip.background = element_blank()) +
  theme(legend.position = "bottom") +
...
ggsave(filename = paste0(plots.path,"/",run_number,"_", reg,"_EnergyService_Industry_totalsteel.png"), width=17, height=10, units = "cm")
