...
results.folder <- data.dir

# path of historical mif
hist.path = paste0("./",run_number, "/historical.mif")
...
df <- df %>%
        calc_addVariable("`SE|Electricity|Other Renewables`" = "`SE|Electricity|Biomass`+`SE|Electricity|Solar|CSP`+`SE|Electricity|Hydro`+`SE|Electricity|Geothermal`",
                               "`SE|Electricity|Other Fossil`" =  "`SE|Electricity|Coal` + `SE|Electricity|Oil`",
                               units = "EJ/yr")
...
df <-  df %>%
 ...
 `Emi|CO2|Energy|Demand|+|Buildings` + `Emi|CO2|Energy|Demand|+|Transport`))/((`FE|Gases|+|Hydrogen` +
 `FE|Gases|+|Fossil` + `FE|Solids|+|Fossil` + `FE|Solids|+|Biomass` + `FE|Liquids|+|Hydrogen` +
`FE|Liquids|+|Fossil` + `FE|Liquids|+|Biomass`)*277777777.78)", ## convert MtCO2 in kgCO2 and EJ into MWh
...
ggsave(plot = p.emi.int,
       filename =  paste0(plots.path, "/Emi_int_FE.png"),
       width=20, height=10, units="cm")
...
ggsave(plot = p.emi.transp,
       filename =  paste0(plots.path, "/Emi_trsp.png"),
       width=20, height=10, units="cm")
