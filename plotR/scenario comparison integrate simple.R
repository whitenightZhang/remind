...
scenarios = c(
  "REMIND_generic_baseline_bit_withoutPlus",
  "REMIND_generic_ndc_bit_withoutPlus",
  "REMIND_generic_1p5c_Elec_bit_NZ_CO2_withoutPlus",
  "REMIND_generic_1p5c_bal_bit_NZ_CO2_withoutPlus",
  NULL)
...
scens_short =
  c(
    "Baseline",
    "NDC",
    "1.5C netzero DirEl Simple",
    "1.5C netzero bal Simple",
    NULL)
...
ggsave(filename = paste0(plots.path, "/", run_number, "_CI_nonElec_lineplot", reg, ".png"), width=20, height=14, units = "cm")
...
df.plot0 = df %>%
  filter(scenario == "1.5C netzero DirEl Simple") %>% 
...
g1 <- ggplot() + geom_line(data= df.plot, aes(x=period, y = value, color = variable)) +
  ggtitle("1.5C netzero DirEl Simple")+
  # xlim(c(2005,2060))+
...
df.sector_emi_check <- df.plot %>% 
  filter(scenario == "1.5C netzero bal Simple",region == "CHA")
...
df.1.5.check = filter(df.plot, grepl("Emi|CO2", variable, fixed=TRUE), period > 2005, period < 2065, scenario == "1.5C netzero bal Simple", region == reg)  
...
df.plot2 = df.plot %>% filter(region == reg, scenario == "1.5C netzero bal Simple", variable == "Emi|CO2|Energy|Demand|CDR")
...
# p<-ggplot() + geom_line(data= df.linecomparison_elshare, aes(x=period, y = value,color = scenario)) +
#   theme_bw() +
...
#   scale_color_manual(values = cbbPalette)
#   
# ggsave(filename = paste0(plots.path,"/", run_number,"_",reg, "_", "FE_elshare.png"),width=18, height=10, units = "cm")
...
df.Tsinghua_2C_b$variable <- "FE|Building"
...
df.Tsinghua_2C = list(df.Tsinghua_2C_i, df.Tsinghua_2C_b,df.Tsinghua_2C_t) %>%
  reduce(full_join) %>%
...
mip::mipArea(df.plot2  %>% filter(period <= 2065), total = df.tot %>% filter(period <= 2065),) +
  theme_bw() +
  theme(strip.background = element_blank()) +
  theme(legend.position = "right") +
  ylab("Final Energy[EJ/yr]") +
...
df.plot = filter(df, variable %in% vars, period <= tmax, region == "CHA" ) %>%
  # factor.data.frame() %>% 
  # arrange(variable) %>% 
  mutate(value = value) %>%
